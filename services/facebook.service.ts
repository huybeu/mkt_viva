...
export class FacebookService {
  list() { return db.facebookPage.findMany({ select: { id: true, pageId: true, name: true, isActive: true, createdAt: true, updatedAt: true }, orderBy: { name: "asc" } }); }
  async connect(input: { pageId: string; name: string; accessToken: string }) {
    ...
    const page = await db.facebookPage.upsert({ where: { pageId: verified.id }, create: { pageId: verified.id, name: verified.name || input.name, accessTokenEncrypted: encrypt(input.accessToken) }, update: { name: verified.name || input.name, accessTokenEncrypted: encrypt(input.accessToken), isActive: true } });
  ...
  async connectFromUserToken(userToken: string) {
    ...
    const connected = [];
    for (const item of payload.data ?? []) connected.push(await this.connect({ pageId: item.id, name: item.name, accessToken: item.access_token }));
    return connected;
  }
  async remove(id: string) { const page = await db.facebookPage.findUnique({ where: { id } }); if (!page) throw new ApiError(404, "FACEBOOK_PAGE_NOT_FOUND", "Không tìm thấy Facebook Page"); await db.facebookPage.delete({ where: { id } }); }
  async publish(postId: string, facebookPageId: string) {
    ...
    if (!page || !page.isActive) throw new ApiError(404, "FACEBOOK_PAGE_NOT_FOUND", "Facebook Page chưa được liên kết");
